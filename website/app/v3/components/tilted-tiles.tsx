...
const tiles = [
  { icon: <Globe2 className="h-4 w-4" />, label: "Монгол хэл дээр" },
  { icon: <Wrench className="h-4 w-4" />, label: "Практик хэрэглээ" },
  { icon: <RefreshCw className="h-4 w-4" />, label: "Шинэчлэгддэг" },
  { icon: <BookOpen className="h-4 w-4" />, label: "Tool биш, чадвар" },
];
      ...
      <div className="grid rotate-[-3deg] grid-cols-2 gap-4">
        {tiles.map((tile, i) => (
          ...
          >
            ...
            <p className="mt-3 text-xs font-medium text-[#b7c6c9]">{tile.label}</p>
          </div>
        ))}
      </div>
    </div>
